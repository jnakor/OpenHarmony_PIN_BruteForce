...
{
    UTILS_LOGI("ThreadPool::ThreadPool %{public}s", myName_.c_str());
}

ThreadPool::~ThreadPool()
{
    UTILS_LOGI("ThreadPool::~ThreadPool %{public}s start", myName_.c_str());
    if (running_) {
        Stop();
    }
    UTILS_LOGI("ThreadPool::~ThreadPool %{public}s end", myName_.c_str());
}
...
uint32_t ThreadPool::Start(int numThreads)
{
    ...
    UTILS_LOGI("ThreadPool %{public}s started", myName_.c_str());
    return ERR_OK;
}
