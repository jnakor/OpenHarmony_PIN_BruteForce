...
ResultCode AddPin(PinEnrollParam *pinEnrollParam, uint64_t *templateId, Buffer *outRootSecret)
{
    ...
    // = RefreshPinDb(templateId, pinEnrollParam->subType);
    // if (ret != RESULT_SUCCESS) {
...
{
    if (salt == NULL) {
        LOG_ERROR("get invalid salt params888.");
        return RESULT_BAD_PARAM;
    }
...
ResultCode CompareData(const uint8_t *inputData, const uint32_t inputDataLen, const uint8_t *storeData,
...
Buffer *GenerateDecodeCredential(const Buffer *deviceKey, const Buffer *pinData)
{
    Buffer *encryptionKey = GenerateEncryptionKey(deviceKey);
    if (!IsBufferValid(encryptionKey)) {
        LOG_ERROR("generate encryptionKey fail.");
